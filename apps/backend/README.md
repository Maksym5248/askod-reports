...
Додаткові операції документів:

- `GET /api/documents/columns` — 43 поля та enum-значення для редакторів.
- `PATCH /api/documents/:id` — `{ version, field, value }`, повний документ у відповіді.
- `POST /api/documents/delete` — `{ items: [{ id, version }] }`, успіх 204.
- `GET /api/documents/:id/history` — імпортовані знімки й ручні зміни.

Список підтримує `search`, `documentType`, `sortBy`, `sortDirection` поряд із пагінацією.
Відповідь включає всі 42 змістові поля, id, createdAt, updatedAt, version.
Конфлікт редагування/версії/ключа — 409; документ відсутній або видалений — 404.
[Правила керування документами](../../docs/document-table.md).
...
показуються в JSON з номером рядка й назвою поля; `issueCount` містить повну кількість,
а `report: { fileName, base64 }` — XLSX з усіма знайденими помилками. Запис та історія — одна транзакція.
...
Вісім полів перевіряються за закритими domain enum-ами. Перелік і правила:
[Довідники даних](../../docs/data-dictionaries.md).
