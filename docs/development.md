...
Smoke test відкриває реальне вікно, перевіряє HTTP-зв’язок із SQLite та ізоляцію Node.
Він використовує тимчасовий ізольований userData та SQLite-базу; знімок зберігається
в `.data/desktop-smoke.png`. Також перевіряються SPA-навігація, reload, persist
налаштувань, пагінація та помилка імпорту через підставлені HTTP-відповіді.
Ізольований профіль дозволяє перевірку поруч із відкритим застосунком.
Тести імпорту працюють у тимчасових каталогах і не змінюють користувацькі документи.
