...
Відповідь списку документів тепер містить усі змістові поля та службові дати/версію.
ORM-поля, хеші й зв’язки не виходять у DTO.
...
GET /api/documents?page=1&pageSize=25
  ...
  → JournalRepository.list: фільтри + count + page query
  → mapper: Prisma → domain
  → documentsSchema: формування DTO
  → React table
```

Логіка пагінації в repository використовує `skip/take`; типовий порядок — дата
реєстрації за спаданням, номер та ID за зростанням. Клієнт може обрати поле й напрям. Кількість і сторінка читаються в транзакції.
...
JSON містить перші 100 помилок, повну кількість та XLSX-звіт із усіма знайденими
помилками нормалізації/повторів. Структурні помилки файла можуть повертатися першими.
Enum-и та контракт звіту описано в [довідниках даних](data-dictionaries.md).
...
Electron smoke test підтверджує запуск, зв’язок із backend та відсутність Node globals.
Окремі [Playwright E2E-сценарії](e2e-testing.md) перевіряють навігацію, імпорт,
редагування, видалення й налаштування в ізольованих профілях.
...
ManageDocuments реалізує ручне редагування, читання історії та групове м’яке видалення.
DocumentChange зберігає before/after ручних операцій. Версія захищає від перезапису
застарілих даних, транзакції забезпечують атомарність групових змін.
createdAt/updatedAt відновлюються зі старої історії під час міграції.
[Детальні правила таблиці й API](document-table.md).
